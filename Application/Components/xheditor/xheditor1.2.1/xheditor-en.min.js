...
"#99FFFF","#33FFFF","#66CCCC","#00CCCC","#339999","#336666","#003333","#CCFFFF","#66FFFF","#33CCFF","#3366FF","#3333FF","#000099","#000066","#CCCCFF","#9999FF","#6666CC","#6633FF","#6600CC","#333399","#330099","#FFCCFF","#FF99FF","#CC66CC","#CC33CC","#993399","#663366","#330033"],Ra=[{n:"p",t:"Paragraph"},{n:"h1",t:"Heading 1"},{n:"h2",t:"Heading 2"},{n:"h3",t:"Heading 3"},{n:"h4",t:"Heading 4"},{n:"h5",t:"Heading 5"},{n:"h6",t:"Heading 6"},{n:"pre",t:"Preformatted"},{n:"address",t:"Address"}],Sa=
[{n:"Arial"},{n:"Arial Narrow"},{n:"Arial Black"},{n:"Comic Sans MS"},{n:"Courier New"},{n:"System"},{n:"Times New Roman"},{n:"Tahoma"},{n:"Verdana"}],M=[{n:"xx-small",wkn:"x-small",s:"8pt",t:"xx-small"},{n:"x-small",wkn:"small",s:"10pt",t:"x-small"},{n:"small",wkn:"medium",s:"12pt",t:"small"},{n:"medium",wkn:"large",s:"14pt",t:"medium"},{n:"large",wkn:"x-large",s:"18pt",t:"large"},{n:"x-large",wkn:"xx-large",s:"24pt",t:"x-large"},{n:"xx-large",wkn:"-webkit-xxx-large",s:"36pt",t:"xx-large"}],Ta=[{s:"Align left",
v:"justifyleft",t:"Align left"},{s:"Align center",v:"justifycenter",t:"Align center"},{s:"Align right",v:"justifyright",t:"Align right"},{s:"Align full",v:"justifyfull",t:"Align full"}],Ua=[{s:"Ordered list",v:"insertOrderedList",t:"Ordered list"},{s:"Unordered list",v:"insertUnorderedList",t:"Unordered list"}],Va={"default":{name:"Default",width:25,height:18,line:6,list:{biggrin:"Big grin",smile:"Smile",titter:"Titter",lol:"Lol",call:"Call",victory:"Victory",shy:"Shy",handshake:"Handshake",kiss:"Kiss",
...
upMultiple:true},B);B=b.settings.plugins;var na=[];if(B){ca=d.extend({},ca,B);d.each(B,function(a){na.push(a)});na=na.join(",")}if(b.settings.tools.match(/^\s*(m?full|simple|mini)\s*$/i)){var Ea=sa[d.trim(b.settings.tools)];b.settings.tools=b.settings.tools.match(/m?full/i)&&B?Ea.replace("Table","Table,"+na):Ea}b.settings.tools.match(/(^|,)\s*About\s*(,|$)/i)||(b.settings.tools+=",About");b.settings.tools=b.settings.tools.split(",");S=ja(S,"abs");var Fa="xheCSS_"+b.settings.skin,ha="xhe"+V+"_container",
Ga="xhe"+V+"_Tool",Ha="xhe"+V+"_iframearea",Ia="xhe"+V+"_iframe",wa="xhe"+V+"_fixffcursor",Z="",xa=S+"xheditor_skin/"+b.settings.skin+"/",oa=Va,Ja=ja(S,"rel")+"xheditor_emot/",ya="";oa=d.extend({},oa,b.settings.emots);if(fa=b.settings.showBlocktag)Z+=" showBlocktag";var pa=[];this.init=function(){d("#"+Fa).length==0&&d("head").append('<link id="'+Fa+'" rel="stylesheet" type="text/css" href="'+xa+'ui.css" />');var a=b.settings.width||A.style.width||N.outerWidth();Q=b.settings.height||A.style.height||
N.outerHeight();if(C(Q,"string"))Q=Q.replace(/[^\d]+/g,"");if(a<=0||Q<=0){alert("Current textarea is hidden, please make it show before initialization xhEditor, or directly initialize the height.");return false}if(/^[0-9\.]+$/i.test(""+a))a+="px";var c='<span class="xheGStart"/>',e,h,o=/Separator|BtnBr/i;d.each(b.settings.tools,function(g,f){e=ca[f];if(f.match(o))c+='<span class="xheGEnd"/>';if(f=="Separator")c+='<span class="xheSeparator"/>';else if(f=="BtnBr")c+="<br />";else{h=e.c?e.c:"xheIcon xheBtn"+
f;c+='<span><a href="javascript:void(0);" title="'+e.t+'" name="'+f+'" class="xheButton xheEnabled" tabindex="-1"><span class="'+h+'"/></a></span>';e.s&&b.addShortcuts(e.s,f)}if(f.match(o))c+='<span class="xheGStart"/>'});c+='<span class="xheGEnd"/><br />';N.after(d('<input type="text" id="'+wa+'" style="position:absolute;display:none;" /><span id="'+ha+'" class="xhe_'+b.settings.skin+'" style="display:none"><table cellspacing="0" cellpadding="0" class="xheLayout" style="width:'+a+";height:"+Q+'px;"><tbody><tr><td id="'+
Ga+'" class="xheTool" style="height:1px;"></td></tr><tr><td id="'+Ha+'" class="xheIframeArea"><iframe frameborder="0" id="'+Ia+'" src="" style="width:100%;"></iframe></td></tr></tbody></table></span>'));J=d("#"+Ga);O=d("#"+Ha);a='<html><head><base /><meta content="text/html; charset=UTF-8" http-equiv="Content-Type"/><link rel="stylesheet" href="'+xa+'iframe.css"/>';var j=b.settings.loadCSS;if(j)if(C(j,"array"))for(var p in j)a+='<link rel="stylesheet" href="'+j[p]+'"/>';else a+=j.match(/\s*<style(\s+[^>]*?)?>[\s\S]+?<\/style>\s*/i)?
j:'<link rel="stylesheet" href="'+j+'"/>';a+='</head><body spellcheck="false" dir="ltr" class="editMode'+Z+'"></body></html>';b.win=P=d("#"+Ia)[0].contentWindow;ma=d(P);try{this.doc=u=P.document;da=d(u);u.open();u.write(a);u.close();if(D)u.body.contentEditable="true";else u.designMode="On"}catch(k){}setTimeout(b.setOpts,300);b.setSource();P.setInterval=null;J.append(c).bind("mousedown contextmenu",function(){return false});J.find(".xheButton").click(function(g){var f=d(this);if(f.is(".xheEnabled")){L=
g;b.exec(f.attr("name"))}return false}).hover(function(g){var f=d(this),l=b.settings.hoverExecDelay,m=T;T=null;if(l==-1||ga||!f.is(".xheEnabled"))return false;if(m&&m>10){ga=true;setTimeout(function(){ga=false},100);return false}var n=f.attr("name");if(ca[n].h!=1){b.hidePanel();return false}if(va)l=0;if(l>=0)ua=setTimeout(function(){L=g;Y={x:L.clientX,y:L.clientY};b.exec(n)},l)},function(){Y=null;ua&&clearTimeout(ua)}).mousemove(function(g){if(Y){var f={x:g.clientX-Y.x,y:g.clientY-Y.y};if(Math.abs(f.x)>
1||Math.abs(f.y)>1){if(f.x>0&&f.y>0){f=Math.round(Math.atan(f.y/f.x)/0.017453293);T=T?(T+f)/2:f}else T=null;Y={x:g.clientX,y:g.clientY}}}});K=d("#xhePanel");$=d("#xheShadow");aa=d("#xheCntLine");ra=d("#xheTempIframe");if(K.length==0){K=d('<div id="xhePanel"></div>').mousedown(function(g){g.stopPropagation()});$=d('<div id="xheShadow"></div>');aa=d('<div id="xheCntLine"></div>');d(document.body).append(K).append($).append(aa);if(D)d(document.body).append(ra=d('<iframe id="xheTempIframe" style="display:none;" />'))}d(document).mousedown(b.clickCancelPanel);
da.mousedown(b.clickCancelPanel);d("#"+ha).show();O.css("height",Q-J.outerHeight());N.hide();N.focus(b.focus);Ba.submit(b.saveResult).bind("reset",b.loadReset);p=d(window);p.unload(b.saveResult).bind("beforeunload",b.saveResult);p.resize(b.fixFullHeight);ma.focus(function(){b.settings.focus&&b.settings.focus()}).blur(function(){b.settings.blur&&b.settings.blur()});W&&ma.click(b.fixAppleSel);da.keydown(b.checkShortcuts).keypress(b.forcePtag).bind("mousedown click",function(g){N.trigger(g.type)});if(D){function q(g){g=
d(g.target);var f;if(f=g.css("width"))g.css("width","").attr("width",f.replace(/[^0-9%]+/g,""));if(f=g.css("height"))g.css("height","").attr("height",f.replace(/[^0-9%]+/g,""))}da.bind("controlselect",function(g){g=g.target;d.nodeName(g,"IMG")&&d(g).unbind("resizeend",q).bind("resizeend",q)})}d("body",da).bind("paste",b.cleanWordPaste);(p=b.settings.shortcuts)&&d.each(p,function(g,f){b.addShortcuts(g,f)});V++;ta=true;if(b.settings.fullscreen)b.toggleFullscreen();else b.settings.sourceMode&&setTimeout(b.toggleSource,
20);return true};this.remove=function(){b.hidePanel();b.saveResult();N.unbind("focus",b.focus);Ba.unbind("submit",b.saveResult).unbind("reset",b.loadReset);var a=d(window);a.unbind("unload",b.saveResult).unbind("beforeunload",b.saveResult);a.unbind("resize",b.fixFullHeight);d("#"+ha).remove();N.show();ta=false};this.loadReset=function(){setTimeout(b.setSource,10)};this.saveResult=function(){b.getSource()};this.cleanWordPaste=function(){if(D&&!H){var a;a=ra[0].contentWindow.document.body;a.innerHTML=
"";a.createTextRange().execCommand("Paste");a=a.innerHTML;if(!a.match(/mso-|MsoNormal/i))return true;if(a.indexOf("&nbsp;")==0)a=a.substring(6);a=b.cleanHTML(a);a=b.formatXHTML(a);a=b.cleanWord(a);b.pasteHTML(a);return false}};this.setCSS=function(a){try{b._exec("styleWithCSS",a,true)}catch(c){try{b._exec("useCSS",!a,true)}catch(e){}}};this.setOpts=function(){if(ta&&!H){b.setCSS(false);try{b._exec("enableObjectResizing",true,true)}catch(a){}if(D)try{b._exec("BackgroundImageCache",true,true)}catch(c){}}};
this.forcePtag=function(a){if(H||a.which!=13||a.shiftKey||a.ctrlKey||a.altKey)return true;a=b.getParent("p,h1,h2,h3,h4,h5,h6,pre,address,div,li");if(a.is("li"))return true;if(b.settings.forcePtag)a.length==0&&b._exec("formatblock","<p>");else{b.pasteHTML("<br />");return false}};this.fixFullHeight=function(){if(!ka&&!W){ea&&O.height("100%").css("height",O.outerHeight()-J.outerHeight());D&&J.hide().show()}};this.fixAppleSel=function(a){a=a.target;if(a.tagName.match(/(img|embed)/i)){var c=b.getSel(),
e=b.getRng();e.selectNode(a);c.removeAllRanges();c.addRange(e)}};this.saveBookmark=function(){H||(X=b.getRng())};this.loadBookmark=function(){if(!(H||!X)){b.focus();if(D)X.select();else{var a=b.getSel();a.removeAllRanges();a.addRange(X)}X=null}};this.focus=function(){H?d("#sourceCode",u).focus():ma.focus();return false};this.setCursorFirst=function(a){b.focus();P.scrollTo(0,0);var c=b.getRng(),e=u.body,h=e,o;if(a&&h.firstChild&&(o=h.firstChild.tagName)&&o.match(/^p|div|h[1-6]$/i))h=e.firstChild;D?
c.moveToElementText(h):c.setStart(h,0);c.collapse(true);if(D)c.select();else{a=b.getSel();a.removeAllRanges();a.addRange(c)}};this.getSel=function(){return P.getSelection?P.getSelection():u.selection};this.getRng=function(){var a=b.getSel(),c;try{c=a.rangeCount>0?a.getRangeAt(0):a.createRange?a.createRange():u.createRange()}catch(e){}c||(c=D?u.body.createTextRange():u.createRange());return c};this.getParent=function(a){var c=b.getRng(),e;if(D)e=c.item?c.item(0):c.parentElement();else{e=c.commonAncestorContainer;
if(!c.collapsed)if(c.startContainer==c.endContainer&&c.startOffset-c.endOffset<2&&c.startContainer.hasChildNodes())e=c.startContainer.childNodes[c.startOffset]}a=a?a:"*";e=d(e);e.is(a)||(e=d(e).closest(a));return e};this.getSelect=function(a){var c=b.getSel(),e=b.getRng(),h=true;h=!e||e.item?false:!c||e.boundingWidth==0||e.collapsed;if(a=="text")return h?"":e.text||(c.toString?c.toString():"");if(e.cloneContents){a=d("<div></div>");(e=e.cloneContents())&&a.append(e);e=a.html()}else e=C(e.item)?e.item(0).outerHTML:
C(e.htmlText)?e.htmlText:e.toString();if(h)e="";e=b.processHTML(e,"read");e=b.cleanHTML(e);return e=b.formatXHTML(e)};this.pasteHTML=function(a,c){if(H)return false;b.focus();a=b.processHTML(a,"write");var e=b.getSel(),h=b.getRng();if(c!=undefined){if(h.item){var o=h.item(0);h=u.body.createTextRange();h.moveToElementText(o);h.select()}h.collapse(c)}if(h.insertNode){h.deleteContents();a=h.createContextualFragment(a);c=a.lastChild;h.insertNode(a);h.setEndAfter(c);h.collapse(false);e.removeAllRanges();
e.addRange(h)}else{if(e.type.toLowerCase()=="control"){e.clear();h=b.getRng()}h.pasteHTML(a)}};this.pasteText=function(a,c){a||(a="");a=b.domEncode(a);a=a.replace(/\r?\n/g,"<br />");b.pasteHTML(a,c)};this.appendHTML=function(a){if(H)return false;b.focus();a=b.processHTML(a,"write");d(u.body).append(a)};this.domEncode=function(a){return a.replace(/[<>]/g,function(c){return{"<":"&lt;",">":"&gt;"}[c]})};this.setSource=function(a){X=null;if(typeof a!="string"&&a!="")a=A.value;if(H)d("#sourceCode",u).val(a);
else{if(b.settings.beforeSetSource)a=b.settings.beforeSetSource(a);a=b.cleanHTML(a);a=b.formatXHTML(a);a=b.cleanWord(a);u.body.innerHTML=b.processHTML(a,"write")}};this.processHTML=function(a,c){if(c=="write"){function e(g,f,l,m){m=m.split(",");if(!m[1]){m[1]=m[0];m[0]=""}if(m[0]=="default")m[0]="";return g.replace(/\s+src\s*=\s*(["']?).*?\1(\s|$|\/|>)/i,"$2").replace(f,' src="'+Ja+(m[0]?m[0]:"default")+"/"+m[1]+'.gif"'+(b.settings.emotMark?' emot="'+(m[0]?m[0]+",":"")+m[1]+'" alt="'+m[1]+'"':""))}
a=a.replace(/<img(?:\s+[^>]*?)?(\s+emot\s*=\s*(["']?)\s*(.*?)\s*\2)(?:\s+[^>]*?)?\/?>/ig,e);function h(g,f,l,m,n,i){return g.replace(l,l+" _xhe_"+m+'="'+i+'"')}a=a.replace(/<(\w+(?:\:\w+)?)(?:\s+[^>]*?)?(\s+(src|href)\s*=\s*(["']?)\s*(.*?)\s*\4)(?:\s+[^>]*?)?\/?>/ig,h);a=a.replace(/<(\/?)del(\s+[^>]*?)?>/ig,"<$1strike$2>");if(ka){a=a.replace(/<(\/?)strong(\s+[^>]*?)?>/ig,"<$1b$2>");a=a.replace(/<(\/?)em(\s+[^>]*?)?>/ig,"<$1i$2>")}else if(W){a=a.replace(/("|;)\s*font-size\s*:\s*([a-z-]+)(;?)/ig,function(g,
f,l,m){for(var n,i=0;i<M.length;i++){g=M[i];if(l==g.n){n=g.wkn;break}}return f+"font-size:"+n+m});a=a.replace(/<strong(\s+[^>]*?)?>/ig,'<span class="Apple-style-span" style="font-weight: bold;"$1>');a=a.replace(/<em(\s+[^>]*?)?>/ig,'<span class="Apple-style-span" style="font-style: italic;"$1>');a=a.replace(/<u(\s+[^>]*?)?>/ig,'<span class="Apple-style-span" style="text-decoration: underline;"$1>');a=a.replace(/<strike(\s+[^>]*?)?>/ig,'<span class="Apple-style-span" style="text-decoration: line-through;"$1>');
a=a.replace(/<\/(strong|em|u|strike)>/ig,"</span>");a=a.replace(/<span((?:\s+[^>]*?)?\s+style="([^"]*;)*\s*(font-family|font-size|color|background-color)\s*:\s*[^;"]+\s*;?"[^>]*)>/ig,'<span class="Apple-style-span"$1>')}else if(D){a=a.replace(/&apos;/ig,"&#39;");a=a.replace(/\s+(disabled|checked|readonly|selected)\s*=\s*[\"\']?(false|0)[\"\']?/ig,"")}a=a.replace(/<a(\s+[^>]*?)?\/>/,"<a$1></a>");if(!W){function o(g,f,l,m){f="";var n,i;if(n=l.match(/font-family\s*:\s*([^;"]+)/i))f+=' face="'+n[1]+'"';
if(n=l.match(/font-size\s*:\s*([^;"]+)/i)){n=n[1].toLowerCase();for(var r=0;r<M.length;r++)if(n==M[r].n||n==M[r].s){i=r+1;break}if(i){f+=' size="'+i+'"';l=l.replace(/(^|;)(\s*font-size\s*:\s*[^;"]+;?)+/ig,"$1")}}if(i=l.match(/(?:^|[\s;])color\s*:\s*([^;"]+)/i)){if(n=i[1].match(/\s*rgb\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/i)){i[1]="#";for(r=1;r<=3;r++)i[1]+=(n[r]-0).toString(16)}i[1]=i[1].replace(/^#([0-9a-f])([0-9a-f])([0-9a-f])$/i,"#$1$1$2$2$3$3");f+=' color="'+i[1]+'"'}l=l.replace(/(^|;)(\s*(font-family|color)\s*:\s*[^;"]+;?)+/ig,
"$1");if(f!=""){if(l)f+=' style="'+l+'"';return"<font"+f+">"+m+"</font>"}else return g}a=a.replace(/<(span)(?:\s+[^>]*?)?\s+style\s*=\s*"((?:[^"]*?;)*\s*(?:font-family|font-size|color)\s*:[^"]*)"(?: [^>]+)?>(((?!<\1(\s+[^>]*?)?>)[\s\S])*?)<\/\1>/ig,o);a=a.replace(/<(span)(?:\s+[^>]*?)?\s+style\s*=\s*"((?:[^"]*?;)*\s*(?:font-family|font-size|color)\s*:[^"]*)"(?: [^>]+)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?)<\/\1>/ig,o);a=a.replace(/<(span)(?:\s+[^>]*?)?\s+style\s*=\s*"((?:[^"]*?;)*\s*(?:font-family|font-size|color)\s*:[^"]*)"(?: [^>]+)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?<\/\1>)*?)<\/\1>/ig,
o)}}else{var j=b.settings.localUrl;function p(g,f,l,m){if(j)m=ja(m,j);return g.replace(new RegExp("\\s+"+f+"\\s*=\\s*([\"']?).*?\\1(\\s|/?>)","ig")," "+f+'="'+m.replace(/\$/g,"$$$$")+'"$2')}a=a.replace(/<(?:\w+(?:\:\w+)?)(?:\s+[^>]*?)?\s+_xhe_(src|href)\s*=\s*(["']?)\s*(.*?)\s*\2(?:\s+[^>]*?)?\/?>/ig,p);if(W){a=a.replace(/("|;)\s*font-size\s*:\s*([a-z-]+)(;?)/ig,function(g,f,l,m){for(var n,i=0;i<M.length;i++){g=M[i];if(l==g.wkn){n=g.n;break}}return f+"font-size:"+n+m});var k=[{r:/font-weight:\sbold/ig,
...
q);a=a.replace(/<(span)(\s+[^>]*?)?\s+class\s*=\s*"Apple-style-span"(\s+[^>]*?)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?<\/\1>)*?)<\/\1>/ig,q)}a=a.replace(/(<\w+(?:\s+[^>]*?)?)\s+(?:_xhe_|_moz_|_webkit_)[^=]+?\s*=\s*(["']?).*?\2(\s|\/?>)/ig,"$1$3");a=a.replace(/(<\w+[^>]*?)\s+class\s*=\s*(["']?)\s*(?:apple|webkit)\-.+?\s*\2(\s|\/?>)/ig,"$1$3");a=a.replace(/<img(\s+[^>]+?)\/?>/ig,function(g,f){f.match(/\s+alt\s*(["']?).*?\1(\s|$)/i)||
(f+=' alt=""');return"<img"+f+" />"})}return a};this.getSource=function(a){var c,e=b.settings.beforeGetSource;if(H){c=d("#sourceCode",u).val();e||(c=c.replace(/(\t*\r?\n\t*)+/g,""))}else{c=b.processHTML(u.body.innerHTML,"read");c=c.replace(/^\s*(?:<(p|div)(?:\s+[^>]*?)?>)?\s*(<br(?:\s+[^>]*?)?>)*\s*(?:<\/\1>)?\s*$/i,"");c=b.cleanHTML(c);c=b.formatXHTML(c,a);c=b.cleanWord(c);if(e)c=e(c)}return A.value=c};this.cleanWord=function(a){if(a.match(/mso-|MsoNormal/i)){var c=b.settings.wordDeepClean;a=a.replace(/(<link(?:\s+[^>]*?)?)\s+href\s*=\s*(["']?)\s*file:\/\/.+?\s*\2((?:\s+[^>]*?)?\s*\/?>)/ig,
...
return false}}};this.uploadInit=function(a,c,e){function h(){a.closest(".xheDialog").bind("dragenter dragover",function(){return false}).bind("drop",function(i){i=i.originalEvent.dataTransfer;var r;if(i&&(r=i.files)&&r.length>0)o(r);return false})}function o(i){n=[];var r,t,z=d('<div style="margin:22px 0;text-align:center;line-height:30px;">File uploading,please wait...<br /></div>'),x='<img src="'+xa+'img/loading.gif">';if(i.nodeType&&!((t=i.files)&&t[0])){if(!j(i.value))return;z.append(x);r=new b.html4Upload(i,
c,p)}else{t||(t=i);var w=t.length;if(!l&&w>1){alert("Please do not upload multiple files.");return}for(i=0;i<w;i++)if(!j(t[i].fileName))return;var y=d('<div class="xheProgress"><div><span>0%</span></div></div>');z.append(y);r=new b.html5Upload(q,t,c,p,function(G){if(G.loaded>=0){var E=Math.round(G.loaded*100/G.total)+"%";d("div",y).css("width",E);d("span",y).text(E+" ( "+Aa(G.loaded)+" / "+Aa(G.total)+" )")}else y.replaceWith(x)})}R=false;b.showModal("File uploading(Esc cancel)",z,320,150,function(){R=
true;r.remove()});r.start()}function j(i){if(e=="*"||i.match(new RegExp(".("+e.replace(/,/g,"|")+")$","i")))return true;else{alert("Upload file extension required for this:"+e);return false}}function p(i,r){var t=Object,z=false,x=b.settings.onUpload;try{t=eval("("+i+")")}catch(w){}if(t.err==undefined||t.msg==undefined)alert(c+" upload interface error!\r\n\r\nreturn error:\r\n\r\n"+i);else if(t.err)alert(t.err);else{n.push(t.msg);z=true}if(!z||r)b.removeModal();if(r&&z){k(n);x&&x(n)}return z}function k(i){if(C(i,
"string"))i=[i];var r=false,t,z=i.length,x,w=[];for(t=0;t<z;t++){x=i[t];x=C(x,"string")?x:x.url;if(x.substr(0,1)=="!"){r=true;x=x.substr(1)}w.push(x)}a.val(w.join(" "));r&&a.closest(".xheDialog").find("#xheSave").click()}var q="filedata",g=d('<span class="xheUpload"><input type="text" style="visibility:hidden;" tabindex="-1" /><input type="button" value="'+b.settings.upBtnText+'" class="xheBtn" tabindex="-1" /></span>'),f=d(".xheBtn",g),l=b.settings.upMultiple;a.after(g);f.before(a);c=c.replace(/{editorRoot}/ig,
S);if(c.substr(0,1)=="!")f.click(function(){R=false;b.showIframeModal("Upload file",c.substr(1),k,null,null,function(){R=true})});else{g.append('<input type="file"'+(l?' multiple=""':"")+' class="xheFile" size="13" name="'+q+'" tabindex="-1" />');var m=d(".xheFile",g),n=[];m.change(function(){o(m[0])});setTimeout(h,10)}};this.html4Upload=function(a,c,e){var h="jUploadFrame"+(new Date).getTime(),o=this,j=d('<iframe name="'+h+'" class="xheHideArea" />').appendTo("body"),p=d('<form action="'+c+'" target="'+
h+'" method="post" enctype="multipart/form-data" class="xheHideArea"></form>').appendTo("body"),k=d(a),q=k.clone().attr("disabled","true");k.before(q).appendTo(p);this.remove=function(){if(o!=null){q.before(k).remove();j.remove();p.remove();o=null}};this.onLoad=function(){e(d(j[0].contentWindow.document.body).text(),true)};this.start=function(){p.submit();j.load(o.onLoad)};return this};this.html5Upload=function(a,c,e,h,o){function j(i,r,t,z){k=new XMLHttpRequest;upload=k.upload;k.onreadystatechange=
function(){k.readyState==4&&t(k.responseText)};if(upload)upload.onprogress=function(x){z(x.loaded)};else z(-1);k.open("POST",r);k.setRequestHeader("Content-Type","application/octet-stream");k.setRequestHeader("Content-Disposition",'attachment; name="'+a+'"; filename="'+i.fileName+'"');k.sendAsBinary?k.sendAsBinary(i.getAsBinary()):k.send(i)}function p(i){o&&o({loaded:f+i,total:l})}for(var k,q=0,g=c.length,f=0,l=0,m=this,n=0;n<g;n++)l+=c[n].fileSize;this.remove=function(){if(k){k.abort();k=null}};
this.uploadNext=function(i){if(i){f+=c[q-1].fileSize;p(0)}if((!i||i&&h(i,q==g)==true)&&q<g)j(c[q++],e,m.uploadNext,function(r){p(r)})};this.start=function(){m.uploadNext()}};this.showIframeModal=function(a,c,e,h,o,j){c=d('<iframe frameborder="0" src="'+c.replace(/{editorRoot}/ig,S)+'" style="width:100%;height:100%;display:none;" /><div class="xheModalIfmWait"></div>');var p=d(c[0]),k=d(c[1]);b.showModal(a,c,h,o,j);p.load(function(){var q=p[0].contentWindow,g=d(q.document);q.callback=function(f){b.removeModal();
e(f)};g.keydown(b.modalEscCheck);p.show();k.remove()})};this.showModal=function(a,c,e,h,o){if(d(".xheModal").length==1)return false;var j,p,k,q,g;e=e?e:b.settings.modalWidth;h=h?h:b.settings.modalHeight;q=b.settings.layerShadow;j=d('<div class="xheModal" style="width:'+(e-1)+"px;height:"+h+"px;margin-left:-"+Math.ceil(e/2)+"px;"+(D&&qa<=7?"":"margin-top:-"+Math.ceil(h/2)+"px")+'">'+(b.settings.modalTitle?'<div class="xheModalTitle"><span class="xheModalClose" title="Close (Esc)"></span>'+a+"</div>":
"")+'<div class="xheModalContent"></div></div>').appendTo("body");k=d('<div class="xheModalOverlay"></div>').appendTo("body");if(q>0)p=d('<div class="xheModalShadow" style="width:'+j.outerWidth()+"px;height:"+j.outerHeight()+"px;margin-left:-"+(Math.ceil(e/2)-q-2)+"px;"+(D&&qa<=7?"":"margin-top:-"+(Math.ceil(h/2)-q-2)+"px")+'"></div>').appendTo("body");d(".xheModalContent",j).css("height",h-(b.settings.modalTitle?d(".xheModalTitle").outerHeight():0)).html(c);if(D&&qa==6)g=d("select:visible").css("visibility",
"hidden");this.removeModal=function(){g&&g.css("visibility","visible");d(document).unbind("keydown",b.modalEscCheck);j.remove();q>0&&p.remove();k.remove();o&&o()};this.modalEscCheck=function(f){if(f.which==27){b.removeModal();return false}};d(document).keydown(b.modalEscCheck);d(".xheModalClose",j).click(b.removeModal);k.show();q>0&&p.show();j.show()};this.showDialog=function(a){var c=d('<div class="xheDialog"></div>');a=d(a);var e=d("#xheSave",a);if(e.length==1){a.find("input[type=text],select").keypress(function(j){if(j.which==
13){e.click();return false}});a.find("textarea").keydown(function(j){if(j.ctrlKey&&j.which==13){e.click();return false}});e.after(' <input type="button" id="xheCancel" value="Cancel" />');d("#xheCancel",a).click(b.hidePanel);if(!b.settings.clickCancelDialog){la=false;var h=d('<div class="xheFixCancel"></div>').appendTo("body").mousedown(function(){return false}),o=O.offset();h.css({left:o.left,top:o.top,width:O.outerWidth(),height:O.outerHeight()})}c.mousedown(function(){ga=true})}c.append(a);b.showPanel(c);
D||setTimeout(function(){c.find("input[type=text],textarea").filter(":visible").filter(function(){return d(this).css("visibility")!="hidden"}).eq(0).focus()},10)};this.clickCancelPanel=function(){la&&b.hidePanel()};this.showPanel=function(a){if(!L.target)return false;K.empty().append(a).css("left",-999).css("top",-999);ba=d(L.target).closest("a").addClass("xheActive");var c=ba.offset();a=c.left;c=c.top;c+=ba.outerHeight()-1;aa.css({left:a+1,top:c}).show();if(a+K.outerWidth()>document.body.clientWidth)a-=
K.outerWidth()-ba.outerWidth();var e=b.settings.layerShadow;e>0&&$.css({left:a+e,top:c+e,width:K.outerWidth(),height:K.outerHeight()}).show();K.css("left",a).css("top",c).show();va=R=true};this.hidePanel=function(){if(R){ba.removeClass("xheActive");$.hide();aa.hide();K.hide();R=false;if(!la){d(".xheFixCancel").remove();la=true}va=ga=false;T=null}};this.exec=function(a){b.hidePanel();b.focus();b.saveBookmark();var c=ca[a];if(!c)return false;if(L==null){L={};var e=J.find(".xheButton[name="+a+"]");if(e.length==
1)L.target=e}if(c.e)c.e.call(b);else{a=a.toLowerCase();switch(a){case "cut":try{u.execCommand(a);if(!u.queryCommandSupported(a))throw"Error";}catch(h){alert("Currently not supported by your browser, use keyboard shortcuts(Ctrl+X) instead.")}break;case "copy":try{u.execCommand(a);if(!u.queryCommandSupported(a))throw"Error";}catch(o){alert("Currently not supported by your browser, use keyboard shortcuts(Ctrl+C) instead.")}break;case "paste":try{u.execCommand(a);if(!u.queryCommandSupported(a))throw"Error";
}catch(j){alert("Currently not supported by your browser, use keyboard shortcuts(Ctrl+V) instead.")}break;case "pastetext":window.clipboardData?b.pasteText(window.clipboardData.getData("Text",true)):b.showPastetext();break;case "blocktag":var p=[];d.each(Ra,function(g,f){p.push({s:"<"+f.n+">"+f.t+"</"+f.n+">",v:"<"+f.n+">",t:f.t})});b.showMenu(p,function(g){b._exec("formatblock",g)});break;case "fontface":var k=[];d.each(Sa,function(g,f){f.c=f.c?f.c:f.n;k.push({s:'<span style="font-family:'+f.c+'">'+
f.n+"</span>",v:f.c,t:f.n})});b.showMenu(k,function(g){b._exec("fontname",g)});break;case "fontsize":var q=[];d.each(M,function(g,f){q.push({s:'<span style="font-size:'+f.s+';">'+f.t+"("+f.s+")</span>",v:g+1,t:f.t})});b.showMenu(q,function(g){b._exec("fontsize",g)});break;case "fontcolor":b.showColor(function(g){b._exec("forecolor",g)});break;case "backcolor":b.showColor(function(g){if(D)b._exec("backcolor",g);else{b.setCSS(true);b._exec("hilitecolor",g);b.setCSS(false)}});break;case "align":b.showMenu(Ta,
function(g){b._exec(g)});break;case "list":b.showMenu(Ua,function(g){b._exec(g)});break;case "link":b.showLink();break;case "img":b.showImg();break;case "flash":b.showEmbed("Flash",'<div>Flash URL:<input type="text" id="xheFlashSrc" value="http://" class="xheText" /></div><div>Dimension:<input type="text" id="xheFlashWidth" style="width:40px;" value="480" /> x <input type="text" id="xheFlashHeight" style="width:40px;" value="400" /></div><div style="text-align:right;"><input type="button" id="xheSave" value="OK" /></div>',
"application/x-shockwave-flash","clsid:d27cdb6e-ae6d-11cf-96b8-4445535400000",' wmode="opaque" quality="high" menu="false" play="true" loop="true"',b.settings.upFlashUrl,b.settings.upFlashExt);break;case "media":b.showEmbed("Media",'<div>Media URL:<input type="text" id="xheMediaSrc" value="http://" class="xheText" /></div><div>Dimension:<input type="text" id="xheMediaWidth" style="width:40px;" value="480" /> x <input type="text" id="xheMediaHeight" style="width:40px;" value="400" /></div><div style="text-align:right;"><input type="button" id="xheSave" value="OK" /></div>',
